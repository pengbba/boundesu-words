...
boundesu-words-xml 是 Boundesu Words SDK 的 XML 转换模块，专门负责将 XML 内容转换为 Microsoft Word DOCX 格式。该模块提供了高效的
XML 解析和转换功能，支持复杂的 XML 结构和优化的转换性能。
...
processElement(rootElement, wordDoc);
...
setPreserveWhitespace(true);        // 保留空白字符
options.

setGenerateTableOfContents(true);   // 生成目录
options.

setIncludeMetadata(true);           // 包含元数据
options.

setValidateXml(true);               // 验证 XML 格式
...
setTableBorderStyle("single");      // 表格边框样式
options.

setTableHeaderStyle("bold");        // 表头样式
